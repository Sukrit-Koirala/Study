...
ct_keys, _ = build_datastore_from_chunks(gpt, chunks["controller_train"], batch_size=256)  # only need CT's hidden states, not its target tokens

n_clusters = 500  # same as the other three methods, for a fair comparison
compressed_keys, compressed_dists = query_kmeans_partition(ds_keys, ds_values, ct_keys, n_clusters=n_clusters, seed=42)

# Honesty check: how many clusters actually have datastore members assigned?
n_nonempty = sum(1 for d in compressed_dists if sum(d.values()) > 0)
print(f"non-empty clusters: {n_nonempty} / {n_clusters}")
...
results = {
    "phase": "C10_query_kmeans_baseline",
    "model": "gpt2",
    "seq_len": 128,
...
save_results("results/query_kmeans_baseline.json", results)
print("raw datastore size:", ds_keys.shape)
print("compressed to n_clusters:", n_clusters, f"({n_nonempty} non-empty)")
print("mean pure GPT NLL (same val positions):", mean_nll_pure_lm)
print("mean query_kmeans DIME NLL:", mean_nll_mixed)
