...
# Build the raw datastore, then compress it via minibatch_kmeans (the tuned "default" method from Phase D step 11)
...
# Tuned hyperparameters from Phase D step 11's grid search (minibatch_kmeans winner)
k, tau, alpha = 20, 2.0, 0.05


def build_features_and_reward(keys, true_targets, p_lm_true, entropy):
    distances, retrieved_dists = query_knn(index, stored_values, keys, k=k)
    _, nll_mixed = mix_dime_and_lm(distances, retrieved_dists, true_targets, p_lm_true, tau=tau, alpha=alpha)
    nll_gpt = -np.log(p_lm_true + 1e-12)
    reward = nll_gpt - nll_mixed

    nearest_distance = distances[:, 0]
    retrieved_top1 = retrieved_dists[:, 0]
...
save_results("results/q_read_baseline.json", results)
