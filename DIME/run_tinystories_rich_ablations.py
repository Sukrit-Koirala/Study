...
def evaluate(keys, values_array, index=None, stored=None):
    if index is None:
        index, stored = build_datastore(keys, values_array)
    distances, retrieved = query_knn(index, stored, val_keys, k=K)
...
base_index, base_values = build_datastore(compressed_keys, np.array(compressed_dists, dtype=object))

nll_original = evaluate(compressed_keys, np.array(compressed_dists, dtype=object), base_index, base_values)
...
for K_trunc in [64, 32, 16, 5]:
    ...
    nll = evaluate(compressed_keys, np.array(trunc_dists, dtype=object), base_index, base_values)
...
nll_majority = evaluate(compressed_keys, np.array(majority_dists, dtype=object), base_index, base_values)
...
nll_global_unigram = evaluate(compressed_keys, np.array(global_dists, dtype=object), base_index, base_values)
...
nll_shuffled_distribution = evaluate(compressed_keys, np.array(shuffled_dist_values, dtype=object), base_index, base_values)
