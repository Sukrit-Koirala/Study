...
# Reuse step 5: build the raw datastore from the datastore split
ds_keys, ds_values = build_datastore_from_chunks(gpt, chunks["datastore"], batch_size=256)

# Phase C step 7: compress via random_partition (sanity control — no smart clustering)
n_clusters = 500  # ~100x fewer stored entries than raw kNN's ~49.7K
compressed_keys, compressed_dists = random_partition(ds_keys, ds_values, n_clusters=n_clusters, seed=42)
...
# Encode val split, batched — need p_true (GPT's own probability) for mixing
...
# Step 6 (generalized): retrieve compressed clusters + mix
k, tau, alpha = 5, 1.0, 0.25
distances, retrieved_dists = query_knn(index, stored_values, val_keys, k=k)

# Sanity check: confirm object-array retrieval actually returns real Counters
...
results = {
    "phase": "C7_random_partition_baseline",
    "model": "gpt2",
    "seq_len": 128,
...
save_results("results/random_partition_baseline.json", results)
...
print("mean random-partition DIME NLL:", mean_nll_mixed)
